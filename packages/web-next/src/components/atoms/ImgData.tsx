...
const ImgData: React.FC<{ type: ApiType; src?: string | null; id?: number | null; slug?: string | null }> = (props) => {
  ...
  if (url) {
    return (
      <a className="flex flex-col p-3 w-[212px] justify-between bg-gray-800 text-white rounded-lg font-mono" target="_blank" href={url} rel="noreferrer">
        <div>
          <b className="uppercase">{props.type.replace('aniList', 'al')}: </b>
          <b>{props.id}</b>
        </div>
        <div>{props.src ? <img alt={`${props.type} ID:${props.id}`} src={props.src} width="212px" height="300px" /> : <b>Image not found</b>}</div>
      </a>
    );
  } else {
    return (
